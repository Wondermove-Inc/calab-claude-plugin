...
# 임계값 설정 (Final로 불변 명시)
TOOL_CALL_THRESHOLD: Final[int] = 50  # 도구 호출 횟수 임계값
FILE_EDIT_THRESHOLD: Final[int] = 20  # 파일 수정 횟수 임계값
LINES_CHANGED_THRESHOLD: Final[int] = 500  # 변경된 라인 수 임계값
SESSION_DURATION_THRESHOLD_MINUTES: Final[int] = 30  # 세션 지속 시간 (분)
SUGGESTION_COOLDOWN_SECONDS: Final[int] = 300  # 재제안 방지 쿨다운 (초)
...
def calculate_usage_percentage(state: Dict) -> float:
    """컨텍스트 사용량 추정 (0.0 ~ 1.0)"""
    # 방어적 코딩: 임계값이 0인 경우 방지
    assert TOOL_CALL_THRESHOLD > 0, "TOOL_CALL_THRESHOLD must be positive"
    assert FILE_EDIT_THRESHOLD > 0, "FILE_EDIT_THRESHOLD must be positive"
    assert LINES_CHANGED_THRESHOLD > 0, "LINES_CHANGED_THRESHOLD must be positive"
    ...
    try:
        ...
        time_ratio = duration_minutes / SESSION_DURATION_THRESHOLD_MINUTES
...
def should_suggest_compact(state: Dict, usage: float) -> Tuple[bool, str]:
    ...
    if state.get("last_suggestion"):
        try:
            ...
            if (datetime.now() - last).total_seconds() < SUGGESTION_COOLDOWN_SECONDS:
                return False, ""
        except ValueError:
            pass
