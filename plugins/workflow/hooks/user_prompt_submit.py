...
# 명령어 패턴 캐시
_command_patterns_cache: Optional[Dict] = None


def load_command_patterns() -> Dict:
    """외부 JSON에서 명령어 패턴 로드 (캐싱 적용)"""
    global _command_patterns_cache
    if _command_patterns_cache is not None:
        return _command_patterns_cache

    patterns_file = HOOK_DIR / 'command_patterns.json'
    if patterns_file.exists():
        try:
            with open(patterns_file, 'r', encoding='utf-8') as f:
                _command_patterns_cache = json.load(f)
                return _command_patterns_cache
        except (json.JSONDecodeError, IOError):
            pass

    # 기본값 (파일이 없거나 로드 실패 시)
    _command_patterns_cache = {"commands": {}, "intent_patterns": {}}
    return _command_patterns_cache
...
def detect_slash_command(prompt: str) -> Optional[dict]:
    """슬래시 명령어 감지 (외부 JSON 파일에서 패턴 로드)"""
    prompt_stripped = prompt.strip()

    # 외부 JSON에서 명령어 패턴 로드
    patterns = load_command_patterns()
    command_patterns = patterns.get("commands", {})
...
def detect_work_intent(prompt: str) -> Optional[dict]:
    ...
    # 2. 자연어 키워드 기반 감지 (외부 JSON에서 패턴 로드)
    ...
    for intent_type, data in intent_patterns.items():
        for keyword in data.get('keywords', []):
            if keyword in prompt_lower:
                detected_intent = {
                    'type': intent_type,
                    'category': data.get('category', intent_type),
                    'keyword': keyword,
                    'is_command': False
                }
                break
...
def extract_task_description(prompt: str) -> str:
    ...
    prompt_stripped = prompt.strip()
    truncate_at = MAX_TASK_DESC_LENGTH - len(TRUNCATE_SUFFIX)

    # 슬래시 명령어인 경우 전체 명령어 반환
    if prompt_stripped.startswith('/'):
        # 명령어와 인자를 포함한 전체 첫 줄 반환
        first_line = prompt_stripped.split('\n')[0].strip()
        if len(first_line) > MAX_TASK_DESC_LENGTH:
            first_line = first_line[:truncate_at] + TRUNCATE_SUFFIX
        return first_line
...
def log_user_prompt(prompt: str, intent: Optional[dict]):
    ...
    # 최근 N개만 유지
    history['prompts'] = history['prompts'][-MAX_PROMPT_HISTORY:]
