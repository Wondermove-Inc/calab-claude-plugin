...
description: |
  ...
  요구사항 명확화, 설계, UX 시나리오를 이슈 description에 통합 작성합니다.
...
5. **이슈 작성**: 위 내용을 이슈 description에 통합 작성
...
불명확한 사항이 있으면 `AskUserQuestion` 도구로 질문합니다:
- 한 번에 2-3개 질문만
- 반드시 선택지 제공
- 코드에서 확인 가능한 질문은 지양 (사용자만 답할 수 있는 비즈니스/요구사항 질문에 집중)
...
이슈 description에 다음 내용을 통합 작성합니다:
...
```bash
bd update <plan-subtask-id> --description "<5단계 내용>"
bd close <plan-subtask-id>
```

## 스킵 판단 기준

요청에 따라 이슈 description의 섹션을 선택적으로 포함합니다:

| 섹션 | 포함 조건 |
|------|----------|
| 요구사항 | 항상 포함 |
| 아키텍처 | 새 기능, 아키텍처 변경 시 |
| 인터페이스 정의 | API/인터페이스 변경 시 |
| UX 설계 | UI 변경 시 |
| 구현 가이드 | 항상 포함 |
| TDD 계획 | 코드 로직 변경 시 |
