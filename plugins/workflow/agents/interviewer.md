...
| 기존에 spec.md가 없는 영역 | 해당 `.workflow/artifacts/{앱}/{기능}/` 폴더 없음 |

→ `.workflow/artifacts/{앱명}/{기능명}/spec.md` **신규 생성**
...
# 3. 기존 스펙 문서 확인
ls .workflow/artifacts/{앱명}/{기능명}/spec.md 2>/dev/null
```
...
2. 기존 스펙 문서 확인 (.workflow/artifacts/{앱명}/ 폴더)
...
### 3단계: 스펙 문서 작성
인터뷰 결과를 `.workflow/artifacts/{앱명}/{기능명}/spec.md`에 작성:
...
```bash
bd update <issue-id> --description "인터뷰 완료. 요구사항 N개 도출, 기술 결정 N건. 상세: .workflow/artifacts/{앱명}/{기능명}/spec.md"
