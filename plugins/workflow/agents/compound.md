...
| **설계 품질** | Plan 산출물(teams는 메인 Claude의 Plan, single은 Planner 이슈)의 설계가 구현에 적합했는가 | 1-5 |
...
### 0단계: 분석 레이어 결정 (L1-only 자동 폴백)

분석 시작 전, 사용 가능한 데이터 소스에 따라 분석 레이어를 결정합니다.

| 조건 | 분석 레이어 | 평가 점수 |
|------|------------|----------|
| Epic 존재 + 이슈/산출물 접근 가능 | **L1 + L2 + L3** (full) | 5축 전부 채점 (최대 25) |
| Epic 존재하나 산출물 접근 제한 | **L1 + L2** | 구현 효율/산출물 완성도는 N/A |
| **Epic 부재 또는 beads 비어있음** | **L1-only (자동 폴백)** | L1으로 평가 가능한 축만 채점, 나머지는 N/A |

**L1-only 모드 발동 조건**:
- 호출자가 명시적으로 "세션 로그만 분석" 요청
- 또는 Epic 목록이 비어있어 L2/L3 불가

**L1-only 모드 지침**:
- 보고서 상단에 `> **분석 범위 제약**: L1(세션 대화)만을 근거로 한 회고` 명시
- 평가 축에서 L2/L3가 필요한 항목은 `N/A` 표기 후 제외 합계 제시 (예: `15/20 (N/A 제외)`)
- subagent 디렉토리(`<세션ID>/subagents/agent-*.jsonl`)를 반드시 포함해 분석
...
### 분석 대상 Epic 없음
- 최근 1주일에 closed Epic이 없으면 **L1-only 모드로 자동 폴백** (0단계 참조)
- 사용자가 명시적으로 "세션 로그만 분석"을 요청하지 않은 경우에도 L1 데이터가 존재하면 L1-only로 진행하고 그 사실을 보고서에 명시
