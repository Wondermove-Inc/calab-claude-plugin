...
───────────────────────────────────────────────────────────────
  /workflow:process [기능]
      ...
      예시: /workflow:process 사용자 인증 시스템

  /workflow:process-plan [기능]
      브레인스토밍 + PRD 문서 생성
      옵션: --brainstorm, --prd
      예시: /workflow:process-plan 사용자 인증 시스템

  /workflow:process-design
      아키텍처 + ERD 설계
      옵션: --arch, --erd
      예시: /workflow:process-design --arch

  /workflow:process-tasks
      태스크 목록 자동 생성
      예시: /workflow:process-tasks

  /workflow:process-build [task-id]
      태스크 구현
      옵션: --tdd (테스트 주도 개발)
      예시: /workflow:process-build TASK-001 --tdd

  /workflow:process-status
      진행 상황 확인
      예시: /workflow:process-status
...
🔄 자연어 사용 예시
───────────────────────────────────────────────────────────────
  "개발 시작해줘"      → /workflow:process
  "기획해줘"           → /workflow:process-plan
  "설계해줘"           → /workflow:process-design
  "태스크 분해해줘"    → /workflow:process-tasks
  "구현해줘"           → /workflow:process-build
  "진행 상황 보여줘"   → /workflow:process-status
...
───────────────────────────────────────────────────────────────
  ...
  또는 /workflow:process 로 전체 프로세스 한번에 실행
