...
INTEGRATIONS_PATH = PROJECT_ROOT / 'plugins' / 'jira' / 'integrations'
...
def detect_changes(old_worktree: dict, new_worktree: dict) -> list[dict]:
    """
    Worktree 변경 감지
    ...
    def extract_tasks(worktree: dict) -> dict[str, dict]:
...
def sync_to_jira(changes: list[dict]) -> None:
    """
    변경사항을 JIRA에 동기화
    ...
    try:
        ...
        from utils import load_config, load_mapping, save_mapping
    except ImportError as e:
        log_sync(f"jira 모듈 로드 실패: {e}", 'ERROR')
        return
        ...
        try:
            if change_type == 'task_start' and auto_sync.get('on_task_start'):
                ...
                try:
                    ...
                    # Blocked 상태 전환 불가능한 워크플로우
                    logger.debug(f"Blocked 상태 전환 실패 (무시됨): {e}")
