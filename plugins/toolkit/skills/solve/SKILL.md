...
$ARGUMENTS에서 문제 설명을 파악하거나, 정보가 불충분하면 다음 내용을 텍스트로 출력하고 사용자 응답을 기다립니다:

```
문제를 정확히 파악하기 위해 추가 정보가 필요합니다. 다음 중 제공 가능한 정보를 알려주세요:
1. 에러 메시지 - 정확한 에러 메시지와 스택 트레이스
2. 발생 시점 - 언제부터 발생했는지
3. 재현 조건 - 재현 가능 여부와 조건
4. 최근 변경사항 - 코드, 설정, 환경 변경 여부
```
