...
def find_task_in_worktree(worktree: dict, task_id: str) -> tuple:
    """
    worktree에서 특정 task를 찾아 반환
    Returns: (epic_idx, story_idx, task_idx, task_obj) or (None, None, None, None)
    """
...
def update_progress(worktree: dict):
    """progress 필드 재계산"""
    total = 0
    done = 0
    in_progress = 0
