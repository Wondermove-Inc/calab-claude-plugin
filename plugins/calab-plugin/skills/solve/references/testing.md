...
```mermaid
graph TD
    RED["🔴 RED (실패)<br/>1. 실패하는 테스트 작성<br/>구현 전에 테스트 먼저<br/>명확한 기대 동작 정의"]
    GREEN["🟢 GREEN (통과)<br/>2. 테스트 통과하는 최소 코드<br/>가장 단순한 구현<br/>빠르게 초록불 만들기"]
    REFACTOR["🔵 REFACTOR (개선)<br/>3. 코드 개선<br/>중복 제거, 가독성 향상<br/>테스트는 계속 통과"]

    RED --> GREEN --> REFACTOR -->|반복| RED
```
