...
```mermaid
graph TD
    S1["1. 키워드 도출<br/>주제에서 검색어 추출"] --> S2["2. 다각도 검색<br/>5-10회 검색 수행"]
    S2 --> S3["3. 정보 수집<br/>관련 페이지 내용 추출"]
    S3 --> S4["4. 분석 정리<br/>핵심 내용 요약"]
    S4 --> S5["5. 보고서 생성<br/>구조화된 결과물"]
```
...
> **리서치 분석 완료 시 반드시 보고서 생성**

| 산출물 | 파일 경로 | 필수 |
|--------|----------|------|
| **리서치 보고서** | `.claude/research/{topic}.md` | ✅ |
