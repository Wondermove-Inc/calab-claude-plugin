...
$ARGUMENTS에서 옵션 확인:
- `--5whys`: Phase 3에서 5 Whys만 집중
- `--rca`: Phase 3에서 RCA 프로세스
- `--hypothesis`: Phase 4 가설 검증 강화
- `--binary`: Binary Search 디버깅 모드
- 옵션 없음: 6단계 전체 실행
...
============================================
 ...
 💡 다음 단계:
 • /solve-report {id} - 상세 보고서 확인
 • /solve-history - 해결 이력 조회
...
- `.claude/skills/problem-solving/SKILL.md` - 스킬 정의
- `.claude/skills/problem-solving/methods/` - 방법론 가이드
